...
#We created another function to delete all of the lines with ';;', ';', 'NA' and blank spaces in the case that the grep function did not work or was not prefered.
...
#We decided to use a pipe with a combination of commands, exemplified below, in order make be more time and space efficient in the case that the sed function was prefered over the grep functio. See below, under Action 3.


#Action 3:

#This command separated the taxonomy categories (Kingdom, Phylum, Class, Order, Family, Genus, Species) listed for each species. These are listed at the very end of the data.

#This command took out the single semicolons and replace them with tabs, to create the space in between each column.
...
#Went to first line of the text file, which provides the titles for each column of data. Added the terms Kingdom, Phylum, Class, Order, Family, Genus, Species, all of which were separated by tabs. This was done for our own organization. It is not necessary for the analysis of the data.
